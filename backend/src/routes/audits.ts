...
router.post('/', requirePermission('audit', 'create'), validate(CreateAuditSchema), (req, res) => {
  const id = uuidv4();
  ...
  logChange({ entityType: 'audit', entityId: id, entityLabel: title, action: 'create', user: req.user! });
...
router.patch('/:id', requirePermission('audit', 'update'), validate(UpdateAuditSchema), (req, res) => {
  ...
  const before = db.prepare('SELECT * FROM audits WHERE id = ?').get(req.params.id) as Record<string, unknown>;
  const sets = Object.keys(updates).map(k => `${k} = ?`).join(', ');
  db.prepare(`UPDATE audits SET ${sets}, updated_at = datetime('now') WHERE id = ?`).run(...Object.values(updates), req.params.id);
  logFieldChanges('audit', req.params.id, (before?.title as string) ?? req.params.id, before, updates, req.user!);
  res.json(db.prepare('SELECT * FROM audits WHERE id = ?').get(req.params.id));
});

// Findings
// SoD: only auditor may create findings; reviewer/compliance_manager may remediate (not auditor)
router.post('/:id/findings', requirePermission('finding', 'create'), validate(CreateFindingSchema), (req, res) => {
  const findingId = uuidv4();
  const { control_id, title, description, severity, status, recommendation, management_response, due_date } = req.body;

  const createFinding = db.transaction(() => {
  ...
  createFinding();
  logChange({ entityType: 'finding', entityId: findingId, entityLabel: title, action: 'create', user: req.user! });
  res.json(db.prepare('SELECT * FROM audit_findings WHERE id = ?').get(findingId));
});

router.patch('/findings/:findingId', requirePermission('finding', 'update'), validate(UpdateFindingSchema), (req, res) => {
  const fields = ['title', 'description', 'severity', 'status', 'recommendation', 'management_response', 'due_date', 'remediated_at'];
  const updates: Record<string, unknown> = {};
  for (const f of fields) {
    if (req.body[f] !== undefined) updates[f] = req.body[f];
  }
  // SoD: marking as remediated requires the separate 'remediate' permission (auditors lack it)
  if (req.body.status === 'remediated') {
    const canRemediate = db.prepare(
      "SELECT id FROM role_permissions WHERE role = ? AND resource = 'finding' AND action = 'remediate'"
    ).get(req.user!.role);
    if (!canRemediate) {
      return res.status(403).json({ error: 'Forbidden', detail: `Role '${req.user!.role}' cannot remediate findings (SoD)` });
    }
    if (!updates.remediated_at) updates.remediated_at = new Date().toISOString();
  }
  if (Object.keys(updates).length === 0) return res.status(400).json({ error: 'No fields' });
  const before = db.prepare('SELECT * FROM audit_findings WHERE id = ?').get(req.params.findingId) as Record<string, unknown>;
  const sets = Object.keys(updates).map(k => `${k} = ?`).join(', ');
  ...
  logFieldChanges('finding', req.params.findingId, (before?.title as string) ?? req.params.findingId, before, updates, req.user!);
